"""
app.py - Diabetes Risk Prediction & Lifestyle Segmentation
Dash web application for Group 18 MLG382
"""
...
# Generate feature columns from dataset at startup
df = pd.read_csv(os.path.join(DATA_DIR, 'Diabetes_and_LifeStyle_Dataset_.csv'))
...
def encode_patient(data_dict):
    ...
    df_final = df_enc.reindex(columns=FEATURE_COLUMNS, fill_value=0)
    return df_final

app.layout = dbc.Container([
    dbc.Row([dbc.Col(html.H1('🩺 Diabetes Risk Predictor', className='text-center my-4 text-primary'))]),
    dbc.Row([dbc.Col(html.P('Enter patient details to predict diabetes risk stage and lifestyle cluster.', className='text-center text-muted mb-4'))]),
        ...
        dbc.CardBody([
            dbc.Row([
                dbc.Col([
                    ...
                    dbc.Label('Gender'),
                    dcc.Dropdown(id='gender', options=[{'label': 'Male', 'value': 'Male'}, {'label': 'Female', 'value': 'Female'}, {'label': 'Other', 'value': 'Other'}], value='Male', clearable=False, className='mb-2'),
                    dbc.Label('Ethnicity'),
                    dcc.Dropdown(id='ethnicity', options=[{'label': 'White', 'value': 'White'}, {'label': 'Black', 'value': 'Black'}, {'label': 'Asian', 'value': 'Asian'}, {'label': 'Hispanic', 'value': 'Hispanic'}, {'label': 'Other', 'value': 'Other'}], value='White', clearable=False, className='mb-2'),
                    dbc.Label('Employment Status'),
                    dcc.Dropdown(id='employment_status', options=[{'label': 'Employed', 'value': 'Employed'}, {'label': 'Unemployed', 'value': 'Unemployed'}, {'label': 'Retired', 'value': 'Retired'}, {'label': 'Student', 'value': 'Student'}], value='Employed', clearable=False, className='mb-2'),
                    dbc.Label('Smoking Status'),
                    dcc.Dropdown(id='smoking_status', options=[{'label': 'Never', 'value': 'Never'}, {'label': 'Former', 'value': 'Former'}, {'label': 'Current', 'value': 'Current'}], value='Never', clearable=False, className='mb-2'),
                ], md=4),
                dbc.Col([
                ...
                dbc.Col([
                    ...
                    dbc.Label('Waist-to-Hip Ratio'),
                    dbc.Input(id='waist_hip', type='number', value=0.85, step=0.01, className='mb-2'),
                    dbc.Label('Family History of Diabetes'),
                    dcc.Dropdown(id='family_history', options=[{'label': 'Yes', 'value': 1}, {'label': 'No', 'value': 0}], value=0, clearable=False, className='mb-2'),
                    dbc.Label('Hypertension History'),
                    dcc.Dropdown(id='hypertension', options=[{'label': 'Yes', 'value': 1}, {'label': 'No', 'value': 0}], value=0, clearable=False, className='mb-2'),
                ], md=4),
            ]),
            dbc.Row([
                dbc.Col([
            ...
            dbc.Row([dbc.Col([dbc.Button('🔍 Predict', id='predict-btn', color='primary', size='lg', className='mt-4 w-100')])])
        ])
...
@app.callback(
    ...
    State('employment_status', 'value'), State('smoking_status', 'value'),
    State('bmi', 'value'), State('physical_activity', 'value'), State('sleep_hours', 'value'),
    State('alcohol', 'value'), State('screen_time', 'value'), State('diet_score', 'value'),
    State('systolic_bp', 'value'), State('diastolic_bp', 'value'), State('glucose_fasting', 'value'),
    State('hba1c', 'value'), State('waist_hip', 'value'), State('family_history', 'value'),
    State('hypertension', 'value'), State('model_choice', 'value'),
    prevent_initial_call=True
)
def predict(n_clicks, age, gender, ethnicity, employment, smoking, bmi, physical_activity,
            sleep_hours, alcohol, screen_time, diet_score, systolic_bp, diastolic_bp,
            glucose_fasting, hba1c, waist_hip, family_history, hypertension, model_choice):
    ...
    patient = {
        ...
        'diet_score': diet_score, 'sleep_hours_per_day': sleep_hours,
        'screen_time_hours_per_day': screen_time,
        'family_history_diabetes': family_history, 'hypertension_history': hypertension,
        'cardiovascular_history': 0, 'bmi': bmi, 'waist_to_hip_ratio': waist_hip,
        'systolic_bp': systolic_bp, 'diastolic_bp': diastolic_bp,
        'heart_rate': 72, 'cholesterol_total': 200, 'hdl_cholesterol': 55,
        'ldl_cholesterol': 120, 'triglycerides': 150, 'glucose_fasting': glucose_fasting,
        'glucose_postprandial': 140, 'insulin_level': 10.0, 'hba1c': hba1c,
        'diabetes_risk_score': 5.0, 'diagnosed_diabetes': 0,
    }
    ...
    proba_fig = go.Figure(go.Bar(
        x=list(classes), y=proba,
        marker_color=[STAGE_COLORS.get(c, '#3498db') for c in classes],
        text=[f'{p*100:.1f}%' for p in proba], textposition='auto',
    ))
    proba_fig.update_layout(title='Prediction Probabilities', xaxis_title='Diabetes Stage', yaxis=dict(range=[0, 1]), template='plotly_white')
    ...
    imp_fig.update_layout(title='Top 10 Feature Importances', xaxis_title='Importance', template='plotly_white')
        ...
        dbc.CardBody([
            dbc.Row([
                dbc.Col([dbc.Card([dbc.CardBody([
                    ...
                    html.P(f'Model: {model_choice.upper()}', className='text-muted small'),
